...
# FastRankerのモード指示
# SKILL.mdの評価ノウハウと組み合わせて使う。速度優先・出力形式の指定のみ。
FAST_RANK_PROMPT = """# モード: 一次評価（速度優先）

判断材料は求人タイトルと概要のみです。
上記のSkillに定義された評価観点を参考にしながら、速度優先で1〜10点をつけてください。
詳細な理由は不要です。
...
# DeepRankerのモード指示
# SKILL.mdの評価ノウハウと組み合わせて使う。全フィールドを使って詳細評価。
DEEP_RANK_PROMPT = """# モード: 詳細評価

上記のSkillに定義されたすべての評価観点を使って、各求人を詳細に評価してください。
マッチング理由は「なぜこのスコアか」を1〜2文で説明してください。
